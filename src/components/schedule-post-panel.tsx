...
export function SchedulePostPanel({ isPosting }: SchedulePostPanelProps) {
  ...
  const removeScheduledPost =
    useScheduledPostsStore.getState().removeScheduledPost;
  const updateScheduledPost =
    useScheduledPostsStore.getState().updateScheduledPost;
  const getScheduledPost = useScheduledPostsStore.getState().getScheduledPost;
  const cleanupExpiredPosts =
    useScheduledPostsStore.getState().cleanupExpiredPosts;
  const cleanupPublishedPosts =
    useScheduledPostsStore.getState().cleanupPublishedPosts;
  const markPostAsPublished =
    useScheduledPostsStore.getState().markPostAsPublished;
  ...
  const checkAndPublishScheduledPosts = React.useCallback(async () => {
    const now = new Date();
    const currentScheduledPosts =
      useScheduledPostsStore.getState().scheduledPosts;
    ...
    for (const post of postsToPublish) {
      try {
        ...
        if (result === "success") {
          ...
          if (
            result.includes("v2") ||
            result.includes("Cannot read properties of null")
          ) {
            errorMessage =
              "Twitter API configuration error. Please contact support or try again later.";
          ...
          console.error(
            `Failed to publish post: ${post.id} - result: ${result}`,
          );
        }
      } catch (error) {
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                {getPlatformIcon(
                  post.selectedAccounts[0]?.platform || "Unknown",
                )}
              </div>
              ...
              <div className="text-xs text-red-600 mb-2 bg-red-50 p-2 rounded border border-red-200">
                ...
                {post.errorMessage.includes("v2") && (
                  <div className="mt-1 text-xs text-gray-600">
                    <strong>Troubleshooting:</strong> This error suggests
                    Twitter API configuration issues. Please check that Twitter
                    API credentials are properly configured on the backend.
                  </div>
                )}
              </div>
            )}
            ...
            <div className="text-xs text-gray-500 mt-1">
              {post.status === "published" ? (
                <span className="text-green-600">
                  Published • Will be removed in{" "}
                  {post.publishedAt
                    ? Math.max(
                        0,
                        Math.ceil(
                          (60 -
                            (Date.now() - post.publishedAt.getTime()) / 1000) /
                            60,
                        ),
                      )
                    : 0}
